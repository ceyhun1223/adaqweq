...
const commands = [
    new SlashCommandBuilder().setName('sunucu').setDescription('mta server status'),
    new SlashCommandBuilder().setName('oyuncu').setDescription('player in game'),
]
...
smithbot.once('ready', () => {
    ...
    setInterval(() => {
        smithmta.query({
            ...
            smithbot.user.setActivity(`Oyuncu : ${state.raw.numplayers}/${state.maxplayers}`);
        }).catch(err => {
            console.log(err);
        });
    }, 5000);
...
smithbot.on('interactionCreate', async smithmsg => {
    ...
    if (commandName === 'sunucu') {
        smithmta.query({
            ...
            .setTitle(state.name)
            .setColor(`BLUE`)
            .addField(`Sunucu Sahibi :`,`<@822238104722210887> - <@910916647747981362> - <@758775600835198977> - <@860510640203628544>`,true)
